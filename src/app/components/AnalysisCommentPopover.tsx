import { X, MoreHorizontal, ArrowUp, Smile, AtSign, Image } from 'lucide-react';
...
interface AnalysisCommentPopoverProps {
  isVisible: boolean;
  onClose: () => void;
...
export function AnalysisCommentPopover({ 
  isVisible, 
  onClose, 
  analysisTitle,
  existingComments = [],
  position 
}: AnalysisCommentPopoverProps) {
      ...
      >
        ...
        <div className="px-3 py-3">
          ...
          <div className="flex items-start gap-2">
            <div className="w-6 h-6 rounded-full flex items-center justify-center shrink-0 text-white font-semibold text-xs bg-teal-600 mt-2">
              B
            </div>
            <div className="flex-1 relative">
              <div className="relative border border-gray-300 rounded-lg bg-gray-50 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-transparent">
                <textarea
                  ref={textareaRef}
                  placeholder={existingComments.length > 0 ? "Reply..." : "Start a comment thread..."}
                  value={replyText}
